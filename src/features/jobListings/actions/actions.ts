...
export async function deleteJobListing(id: string) {
  ...
  const nextListing = await db.query.JobListingTable.findFirst({
    where: and(eq(JobListingTable.organizationId, orgId), not(eq(JobListingTable.id, id))),
    columns: { id: true },
    orderBy: desc(JobListingTable.createdAt),
  })

  redirect(nextListing ? `/employer/job-listings/${nextListing.id}` : "/employer/job-listings/new")
}
